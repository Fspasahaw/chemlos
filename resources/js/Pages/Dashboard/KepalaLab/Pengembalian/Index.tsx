...
export default function Index() {
    ...
    const apply = (params: Record<string, string>) => router.get('/dashboard/kepala-lab/pengembalian', params, { preserveState: true, preserveScroll: true, replace: true });

    return (
        <>
            <Head title="Pengembalian" />
            <div className="mb-6">
                <h1 className="text-2xl font-bold">Pengembalian Alat</h1>
                <p className="text-slate-500 dark:text-slate-400">Pantau status pengembalian di laboratorium yang dikelola.</p>
            </div>
            <Card>
                            ...
                            {items.data.length === 0 ? <EmptyTable colSpan={7} message="Tidak ada data." /> : items.data.map((item: any) => {
                                ...
                                return (
                                    <tr key={item.id} className="border-t border-slate-200/80 dark:border-slate-800/80">
                                        ...
                                        <td className="px-4 py-3">{item.user?.nama_lengkap ?? '-'}{item.user?.npm_nip ? ` (${item.user.npm_nip})` : ''}</td>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <Tooltip content="Lihat detail">
                                                <Link href={`/dashboard/kepala-lab/peminjaman/${item.id}`} className="rounded-lg p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20">
