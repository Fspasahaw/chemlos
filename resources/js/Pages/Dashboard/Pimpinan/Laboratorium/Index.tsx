...
export default function Index() {
    ...
    const apply = (params: Record<string, string>) => router.get('/dashboard/pimpinan/laboratorium', params, { preserveState: true, preserveScroll: true, replace: true });

    return (
        <>
            <Head title="Laboratorium" />
            <div className="mb-6">
                <h1 className="text-2xl font-bold">Laboratorium</h1>
                <p className="text-slate-500 dark:text-slate-400">Tampilan data laboratorium (read-only).</p>
            </div>
            <Card>
                        ...
                        <tbody>
                            {items.data.length === 0 ? <EmptyTable colSpan={6} message="Tidak ada data." /> : items.data.map((item: any) => (
                                <tr key={item.id} className="border-t border-slate-200/80 dark:border-slate-800/80">
                                    ...
                                    <td className="px-4 py-3">{item.alats_count ?? 0}</td>
                                    <td className="px-4 py-3">
                                        <Link href={`/dashboard/pimpinan/laboratorium/${item.id}`} className="inline-flex items-center gap-1 rounded-lg bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/20 dark:hover:bg-indigo-900/30"><Eye className="h-3 w-3" /> Detail</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
