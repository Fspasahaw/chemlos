...
export default function Index() {
    ...
    const apply = (params: Record<string, string>) => router.get('/dashboard/pimpinan/kerusakan', params, { preserveState: true, preserveScroll: true });
    ...
    return (
        <>
            ...
            <div className="mb-6">
                ...
                <p className="text-slate-500 dark:text-slate-400">Tampilan data kerusakan (read-only).</p>
            </div>
            <Card>
                            ...
                            {items.data.length === 0 ? <EmptyTable colSpan={7} message="Tidak ada data." /> : items.data.map((item: any) => {
                                ...
                                return (
                                    <tr key={item.id} className="border-t border-slate-200/80 dark:border-slate-800/80">
                                        ...
                                        <td className="px-4 py-3">{item.pelapor?.nama_lengkap ?? '-'}</td>
                                        ...
                                        <td className="px-4 py-3"><Link href={`/dashboard/pimpinan/kerusakan/${item.id}`} title="Lihat detail" className="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"><Eye className="h-4 w-4 text-slate-600" /></Link></td>
                                    </tr>
                                );
                            })}
                        </tbody>
