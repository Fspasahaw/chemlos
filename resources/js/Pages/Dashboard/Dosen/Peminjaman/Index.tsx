...
export default function Index() {
    ...
    const submitReject = (p: Peminjaman) => {
        if (!alasan) return;
        router.post(`/dashboard/dosen/peminjaman/${p.id}/reject`, { alasan_penolakan: alasan }, { preserveScroll: true });
    };
            ...
            accessor: (row: Peminjaman) => (
                <div className="flex flex-wrap items-center justify-end gap-2">
                    <Tooltip content="Lihat detail">
                        <Link href={`/dashboard/dosen/peminjaman/${row.id}`} className="rounded-lg p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20">
                            <Eye className="h-4 w-4" />
                        </Link>
                    </Tooltip>
                    {row.status === 'menunggu_dosen' && (
                        rejectId === row.id ? (
                            <div className="flex items-center gap-2">
                                <input value={alasan} onChange={(e) => setAlasan(e.target.value)} placeholder="Alasan" className="w-40 rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs dark:border-slate-700 dark:bg-slate-800" />
    ...
    return (
        <>
            ...
            <DataTable<Peminjaman>
                ...
                emptyText="Tidak ada peminjaman menunggu."
            />
