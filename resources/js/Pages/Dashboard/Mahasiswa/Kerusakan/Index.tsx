...
export default function Index() {
    ...
    const apply = (params: Record<string, string>) => router.get('/dashboard/mahasiswa/kerusakan', params, { preserveState: true, preserveScroll: true, replace: true });
    ...
    return (
        <>
            ...
            <div className="mb-6">
                ...
                <p className="text-slate-500 dark:text-slate-400">Kerusakan alat terkait peminjaman Anda.</p>
            </div>
            <Card>
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200/80 dark:border-slate-800/80">
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-800">
                            <tr><th className="px-4 py-3 text-left font-semibold">Alat</th><th className="px-4 py-3 text-left font-semibold">Kode Peminjaman</th><th className="px-4 py-3 text-left font-semibold">Kondisi</th><th className="px-4 py-3 text-left font-semibold">Status</th><th className="px-4 py-3 text-left font-semibold">Tanggal</th><th className="px-4 py-3 text-left font-semibold">Aksi</th></tr>
                        </thead>
                        <tbody>
                            {items.data.length === 0 ? <EmptyTable colSpan={6} message="Tidak ada data." /> : items.data.map((item: any) => {
                                ...
                                return (
                                    <tr key={item.id} className="border-t border-slate-200/80 dark:border-slate-800/80">
                                        ...
                                        <td className="px-4 py-3">{item.peminjaman?.kode ?? '-'}</td>
                                        ...
                                        <td className="px-4 py-3"><Link href={`/dashboard/mahasiswa/kerusakan/${item.id}`} title="Lihat detail" className="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"><Eye className="h-4 w-4 text-slate-600" /></Link></td>
                                    </tr>
                                );
                            })}
                        </tbody>
