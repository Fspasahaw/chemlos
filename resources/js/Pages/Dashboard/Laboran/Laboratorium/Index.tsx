...
export default function Index() {
    ...
    const apply = (params: Record<string, string>) => router.get('/dashboard/laboran/laboratorium', params, { preserveState: true, preserveScroll: true, replace: true });
                                ...
                                <tr key={item.id} className="border-t border-slate-200/80 dark:border-slate-800/80">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            <Link href={`/dashboard/laboran/laboratorium/${item.id}`} title="Lihat detail" className="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"><Eye className="h-4 w-4 text-slate-600" /></Link>
                                            <Link href={`/dashboard/laboran/laboratorium/${item.id}/edit`} className="inline-flex items-center gap-1 rounded-lg bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/20"><Edit className="h-3 w-3" /> Edit</Link>
                                        </div>
                                    </td>
                                </tr>
                            ))}
