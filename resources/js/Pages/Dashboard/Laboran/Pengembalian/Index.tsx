...
import { CheckCircle, Eye, Wallet } from 'lucide-react';
...
interface Peminjaman {
    ...
    pengembalian?: { id: number; total_denda: number; denda_dibayar: number | null };
}
...
export default function Index() {
    ...
    const form = useForm({
        waktu_pengembalian: defaultWaktu(),
        kondisi_umum: '',
    ...
    useEffect(() => {
        if (activeItem) {
            ...
            form.setData({ waktu_pengembalian: defaultWaktu(), kondisi_umum: '', foto_bukti: null, detail });
        }
    }, [active]);
    ...
    const submit = (id: number) => {
        form.post(`/dashboard/laboran/pengembalian/${id}`, { forceFormData: true, preserveScroll: true });
    };

    const bayarDenda = (p: any) => {
        if (!p.pengembalian) return;
        router.post(`/dashboard/laboran/pengembalian/${p.pengembalian.id}/bayar-denda`, {}, { preserveScroll: true });
    };
        ...
        { header: 'Periode', accessor: (p: Peminjaman) => `${formatDate(p.tanggal_mulai)} s/d ${formatDate(p.tanggal_selesai)}` },
        {
            ...
            accessor: (p: Peminjaman) => (
                <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    {p.pengembalian && Number(p.pengembalian.total_denda) > 0 && Number(p.pengembalian.denda_dibayar ?? 0) < Number(p.pengembalian.total_denda) && (
                        <ConfirmActionButton
                            icon={<Wallet className="h-4 w-4" />}
                            label="Tandai denda lunas"
                            description={`Tandai denda peminjaman ${p.kode} sebesar ${formatRupiah(p.pengembalian.total_denda)} sebagai lunas?`}
                            variant="warning"
                            confirmLabel="Ya, Lunas"
                            className="text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30"
                            onConfirm={() => bayarDenda(p)}
                        />
                    )}
                        ...
                        {activeItem.details.map((d: Peminjaman['details'][0]) => (
                            <div key={d.id} className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/50">
                                ...
                                <div className="mt-2 grid gap-3 md:grid-cols-3">
                                    <Select options={kondisiOptions} value={form.data.detail[d.id]?.kondisi_pengembalian ?? 'baik'} onChange={(e) => setDetail(d.id, 'kondisi_pengembalian', e.target.value)} label="Kondisi" />
