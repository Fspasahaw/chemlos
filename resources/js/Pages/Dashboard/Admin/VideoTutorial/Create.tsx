...
export default function Create() {
    const { alatOptions } = usePage().props as any;
    const { data, setData, post, processing, errors } = useForm({
        judul: '',
        deskripsi: '',
        jenis: 'aplikasi' as 'aplikasi' | 'alat',
        alat_id: '',
        sumber: 'youtube' as 'youtube' | 'url_eksternal' | 'upload',
        url: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/dashboard/admin/video-tutorial', { forceFormData: data.sumber === 'upload' && data.file !== null });
    };
    ...
    return (
        <>
            <Head title="Tambah Video Tutorial" />
            <div className="mb-6">
                ...
                <h1 className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">Tambah Video Tutorial</h1>
            </div>
            <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200/80 bg-white p-6 dark:border-slate-800/80 dark:bg-slate-900">
                ...
                {data.sumber !== 'upload' ? (
                    ...
                    <FileUpload label="File Video *" accept="video/*" value={data.file} onChange={(file) => setData('file', file)} error={errors.file} />
                )}
