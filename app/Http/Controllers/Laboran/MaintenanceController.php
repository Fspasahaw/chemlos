...
class MaintenanceController extends Controller
{
    ...
    private function belongsToManagedLab(MaintenanceAlat $maintenance): bool
    {
        return in_array($maintenance->laboratorium_id, $this->labIds(), true);
    }

    protected function viewName(): string
    {
        return 'Dashboard/Laboran/Maintenance/Index';
    }
    ...
    public function index(Request $request)
    {
        ...
        $query = MaintenanceAlat::with('alat:id,nama,kode,laboratorium_id', 'alat.laboratorium:id,nama', 'laboratorium:id,nama', 'laboran:id,nama_lengkap', 'kerusakan')
    ...
    public function show(MaintenanceAlat $maintenance)
    {
        ...
        if (! $this->belongsToManagedLab($maintenance)) {
            abort(403);
        }
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($data) {
            ...
            NotifikasiService::kirim(
                ...
                ['kode' => $alat->kode, 'alat' => $alat->nama, 'laboratorium' => $maintenance->laboratorium->nama ?? null, 'teknisi' => $maintenance->teknisi, 'tanggal_mulai' => $maintenance->tanggal_mulai->toDateString()]
            );
            ...
            if ($maintenance->kerusakan?->peminjaman?->user_id) {
                NotifikasiService::kirim(
                    ...
                    ['kode' => $maintenance->kerusakan->peminjaman->kode, 'alat' => $alat->nama]
                );
            }
    ...
    public function start(MaintenanceAlat $maintenance)
    {
        ...
        NotifikasiService::kirim(
            $maintenance->laboran_id,
            'Maintenance Dimulai',
            "Maintenance alat {$alat->nama} ({$alat->kode}) telah dimulai.",
            'maintenance',
            '/dashboard/laboran/maintenance',
            ['alat' => $alat->nama, 'laboratorium' => $maintenance->laboratorium->nama ?? null, 'teknisi' => $maintenance->teknisi]
        );
    ...
    public function cancel(MaintenanceAlat $maintenance)
    {
        ...
        return DB::transaction(function () use ($maintenance) {
            ...
            // Maintenance yang dibuat dari kerusakan pengembalian sudah memindahkan stok saat pengembalian.
            // Jangan kembalikan stok jika berasal dari peminjaman; biarkan kerusakan tetap mencatat unit maintenance.
